document.addEventListener("DOMContentLoaded", () => {
          ...
          <div class="participants">
            ...
            <ul class="participant-list">
              ${details.participants.length > 0 ? details.participants.map(email => `
                <li>
                  <span>${email}</span>
                  <button type="button" class="participant-remove-btn" data-activity="${encodeURIComponent(name)}" data-email="${encodeURIComponent(email)}" aria-label="Remove ${email}">✕</button>
                </li>
  ...
  activitiesList.addEventListener("click", async (event) => {
    if (!event.target.classList.contains("participant-remove-btn")) {
      return;
    }
